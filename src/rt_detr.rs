...
fn thresholded_argmax(a: Array<f32, Ix2>, threshold: f32) -> Array1<u32> { // takes argmax of rows above threshold // https://stackoverflow.com/a/57963733/15275714
...
impl RtDetr {
    ...
    pub fn process(self) -> Result<(), Box<dyn Error>> {
        ...
        let outputs_view = outputs.view().clone().into_owned();
        
        let output_view = outputs_view.slice(s![0,..,..]).view().clone().into_owned();
        ...
        let confident = thresholded_mask(output_view.slice(s![..,4..]).clone().into_owned(), self.threshold);
        let confident_indices = thresholded_argmax(output_view.slice(s![..,4..]).clone().into_owned(), self.threshold);
        // println!("{:?}", confident);
        
        let confident_output_view = apply_mask(&output_view, &confident);
        println!("{:?}", confident_output_view);

        let relevant_output = apply_mask(&output_view, &confident);
        ...
        //     let (i_cx, i_cy, i_w, i_h) = (r_cx * image_size[0], r_cy * image_size[1], r_w * image_size[0], r_h * image_size[1]); // input image coordinates
        //     let (o_cx, o_cy, o_w, o_h) = (r_cx * image_size[0], r_cy * image_size[1], r_w * image_size[0], r_h * image_size[1]); // original image coordinates

        //     let (x_l, x_r, y_t, y_b) = center_coords_to_box_coords(cx, cy, w, h);
                
        //     let indices = thresholded_argmax(box_pred.into_owned(), 0.90);
